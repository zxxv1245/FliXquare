...
- [x]  (24.05.20 주말 간 진행사항 공유, 오늘 진행할 리스트 업)
    ...
    - [ ]  서로 비로그인, 로그인 페이지 구성하기
        - [ ]  비로그인 페이지 진행사항 :: 장르 front-back update, 추가적으로 프로필이나 게시판 페이지
            - [x]  loginView(back,front)
            - [x]  signupView(back,front)
            - [x]  logout(front)
            - [x]  장르 front-back 연결 (수고했설)
            - [ ]  게시판 페이지
        - [ ]  로그인 페이지 진행사항 :: 메인 MovieView(인기영화, 장르쪽), 카테고리 section, 페이지화
            - [ ]  메인 MovieView (Latest, Popular) [caroseul slide 시작 못함]
            - [x]  카테고리에 있는 Section
            - [ ]  Section별 페이지화 (20% 진행(nav 제작 완료), watched (적용 중…), v-for, splice로 렌더되는 것 조절하기 )
    - [x]  ERD 수정 (물리 이름, 타입, 테이블 추가 등)
- [ ]  (24.05.21 로그인 이후 메인 페이지 구성 핵심, 게시판 수정 사항 확인하기 진행할 목록 리스트 업)
    - [ ]  서로 비로그인, 로그인 페이지 구성하기
        - [ ]  비로그인 페이지 진행사항 ::
            - [ ]  
        - [ ]  로그인 페이지 진행사항 ::
